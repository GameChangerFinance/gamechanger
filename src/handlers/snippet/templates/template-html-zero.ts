...
<head>
    ...
    <script>
        ...
        async function main() {
            ...
            handleSetEncoder = function (codec) {
                useCodec = codec;
                updateUI();
                return false;
            };
            ...
            async function updateUI() {
                ...
                try {
                    ...
                    error += 'Failed to decode results. ' + errMessage;
                    console.error(err);
                }
                ...
                try {
                    ...
                    error += 'Failed to build URL. ' + errMessage;
                    console.error(err);
                }
                ...
                encodersBox.innerHTML += $#___ENCODINGS_ARRAY___#$
                    .map(function (codec) {
                        let style = '';
                        if (codec === useCodec) {
                            style = 'style="font-weight:bold;"';
                        }
                        return '<a href="#" class="a-unstyled" ' + style + ' onclick="return handleSetEncoder(\'' + codec + '\')">' + codec + '</a>';
                    })
                    .join(' | ');
            }
        ...
        function urlSafeBase64Decode(safeBase64, padding) {
        ...
        const codecs = {
            gzip: {
                ...
                encoder: async function (code) {
                    ...
                    const encodedData = await compress(json, 'gzip');
                    const buffer = await encodeByteArray(encodedData);
                    const base64String = urlSafeBase64Encode(String(buffer));
                    return '1-' + base64String;
                },
                decoder: async function (msg) {
                    const base64url = msg.replace('1-', '');
                    const base64 = urlSafeBase64Decode(base64url, false);
                    const encodedData = await base64ToBytes(base64);
                    const json = await decompress(encodedData, 'gzip');
            ...
            base64url: {
                ...
                encoder: async function (code) {
                    const json = JSON.stringify(code);
                    const base64String = base64urlEncode(json);
                    return '0-' + base64String;
                },
                decoder: async function (msg) {
                    const base64String = msg.replace('0-', '');
                    const json = base64urlDecode(base64String);
                    return JSON.parse(json);
                }
            }
        };

        function gcEncoder(obj, codec) {
            const selectedCodec = codec || 'gzip';
            return codecs[selectedCodec].encoder(obj);
        }
        ...
        function gcDecoder(msg, codec) {
            ...
            if (!selectedCodec) {
                Object.keys(codecs).forEach(function (codecName) {
                    const header = codecs[codecName].header;
                    if (msg.indexOf(header) === 0) {
                        selectedCodec = codecName;
                    }
                });
            }
            return codecs[selectedCodec].decoder(msg);
        }
    ...
    <style>
        ...
        a:link { color: rgb(174, 47, 174); }
        a:visited { color: rgb(76, 122, 171); }
        a:hover { color: rgb(203, 64, 215); }
        a:active { color: blue; }
        #errorBox { color: #f58000; font-weight: bold; }
        .console {
            overflow: auto;
            text-align: left;
        ...
        .flexrow { display: flex; justify-content: center; gap: 8px; margin: 10px; }
        form { width: 100%; }
        .a-unstyled, .a-unstyled > * { color: inherit; text-decoration: none; }
        ...
        * { scrollbar-width: 10px!important; scrollbar-color: gray rgb(30, 30, 30,0); }
        *::-webkit-scrollbar { width: 10px!important; }
        *::-webkit-scrollbar-track { background: rgb(30, 30, 30,0); }
        *::-webkit-scrollbar-thumb { background-color: gray; border-radius: 10px; border: 3px solid rgb(30, 30, 30,0); }
    </style>
</head>
...
<body>
    <div class="box">
        <h1>$#___TITLE___#$</h1>
        <p><i>$#___DESCRIPTION___#$</i></p>

        <div id="dappConnectorBox">
            <a href="#" id="connectBtn" class="button a-unstyled">Loading....</a>
        </div>
